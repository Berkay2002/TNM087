...
n = floor(n_formula);
if mod(n, 2) == 0
    n = n + 1; % Ensuring the filter size is odd
end
%% Construct the box kernel here:

fbox = ones(n) / (n^2); % Normalizing the box filter kernel


%% Apply the box filter kernel to the image here:

im_filt = imfilter(im, fbox, 'symmetric'); % Use 'symmetric' to avoid border effects
...
o_thresh = im_filt > threshold_value; % Binary image after thresholding
...
% To eliminate an object of size 2x2 pixels, the smallest kernel we can use
% is 7x7 pixels according to n_formula. If the object of size 2x2 is close
% to a larger object, the kernel will include some values from the larger
% object as well. Therefore, the average will be larger, and the function
% will believe that the object is larger than it actually is.
%